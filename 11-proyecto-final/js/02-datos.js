...
(function (TIENDA) {
  ...
  */
  ...
  // Object.freeze impide modificar el array (añadir, quitar, reordenar).
  // ⚠️ OJO: freeze es SUPERFICIAL. Congela el array, pero no los objetos que
  // hay dentro. Para congelarlos todos hay que recorrerlos uno a uno:
  ...
  */
  function obtenerCategorias() {
    const todas = CATALOGO.map((producto) => producto.categoria);
    const sinRepetir = [...new Set(todas)];
    return sinRepetir.sort((a, b) => a.localeCompare(b, TIENDA.utiles.IDIOMA));
  ...
  */
  function cargarCatalogo({ fallar = false, retardo = RETARDO_CARGA } = {}) {
    ...
    return new Promise((resolve, reject) => {
      // setTimeout NO es JavaScript puro: lo proporciona el navegador. Pone la
      // función en la cola de tareas y el motor la ejecuta cuando termina todo
      // lo síncrono. Ese es el famoso "event loop" del proyecto 09.
      setTimeout(() => {
        if (fallar) {
        ...
        // Clonamos cada producto para que nadie pueda tocar el original.
        //
        ...
        // Aquí eso afectaría al array `colores`, así que lo copiamos aparte
        // con [...producto.colores]. Ahora sí es una copia independiente.
        //
  ...
  imprimir(
    `[${hora()}] 02-datos.js cargado. ${CATALOGO.length} productos y ` +
    `${obtenerCategorias().length} categorías disponibles.`
  );
})(window.TIENDA);
